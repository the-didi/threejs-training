...
function IDW(data, options) {
  ...
  if (positions.length !== values.length)
    throw new Error("positions and values must have the same length");
  ...
  this.outerDistFunction =
    options.outerDistFunction || ((arr) => Math.sqrt(IDW.sum(arr)));
  ...
  this.positions.forEach((p) => {
    p.forEach((coordinate, i) => {
      ...
      if (!(coordinate >= ext[i][0] && coordinate <= ext[i][1])) {
        throw new Error(
          `Position at index ${i} is outside the bounds specified in periodicExtent.`
        );
      }
    });
  });
 ...
 */
IDW.prototype.setDistanceFunctions = function (
  innerDistFunction,
  outerDistFunction
) {
  if (typeof innerDistFunction !== "function")
    throw new Error("innerDistFunction must be a function");
  if (typeof outerDistFunction !== "function")
    throw new Error("outerDistFunction must be a function");
...
IDW.prototype.useEuclideanDistance = function () {
  this.setDistanceFunctions(
    (d) => d * d,
    (arr) => Math.sqrt(IDW.sum(arr))
  );
};
...
IDW.prototype.useMinkowskiDistance = function (power = 2) {
  this.setDistanceFunctions(
    (d) => Math.pow(Math.abs(d), power),
    (arr) => Math.pow(IDW.sum(arr), 1 / power)
  );
};
...
IDW.prototype.setWeightFunction = function (weightFunction) {
  if (typeof weightFunction !== "function")
    throw new Error("weightFunction must be a function");
...
IDW.prototype.setDenominatorOffset = function (denominatorOffset) {
  if (typeof denominatorOffset !== "number")
    throw new Error("denominatorOffset must be a number");
...
IDW.prototype.setPeriodicSmoothing = function (smoothing) {
  if (typeof smoothing !== "number")
    throw new Error("smoothing must be a number");
...
IDW.prototype._standardDistance = function (p1, p2) {
  const diffArray = Array(this.dim)
    .fill()
    .map((_, i) => {
      let d = p2[i] - p1[i];
...
IDW._squareEase = function (value, wStart = 0.05, wEnd = 0.05) {
  if (wStart + wEnd > 1)
    throw new Error("The sum of wStart and wEnd cannot exceed 1");
  if (value <= 0) return 0;
  if (value > 1) return 1;
...
IDW.prototype.evaluate = function (position, power = 2) {
  ...
  if (this.hasCustomWeightFunction)
    weights = weights.map((w) => this.weightFunction(w));
  ...
  const weightedAverage = weights.reduce(
    (sum, w, i) => ((sum += w * this.values[i]), sum),
    0
  );
